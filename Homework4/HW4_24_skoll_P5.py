...
while code == 0:
    try:
        ...
        if p1Name == "DEBUGSTOP":  # Debugging Stop
            break
        
        while rnd < 3:
            while code == 0:  # Keep on turn until correct input entered
                p1Turn = input(p1Name + "'s turn: ")  # P1 Turn Indicator
                p1Turn = p1Turn.upper()
        ...
        if playIn.upper() == "Y":
            ...
            if newNames.upper() == "Y":
                namer = 0
                rnd, p1Wins, p2Wins = 0, 0, 0, 0  # Reset values
